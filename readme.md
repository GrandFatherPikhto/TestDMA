...
На практике это зависит от системной шины (AHB), размера данных, и конкуренции за доступ к памяти, поэтому лучше тактировать передачу при помощи таймера, иначе, частота семплирования будет &laquo;плавать&raquo;
...
Начиная с отметки в 10 мкс (сто тактов) каждые 50 тактов (5 мкс) полярности пинов PE11 и PE12 меняются
Начиная с 60 мкс генерация сигнала на пинах PE11 и PE12 прекращается.
Соответственно, можно настроить любые пины GPIOE.

Конечно, можно сократить количество точек, уменьшить частоту считывания данных DMA. Однако, если нужно будет динамически настраивать частоту семплирования, скажем от 100 КГц до 1МГц и с большой точностью изменять конфигурацию сигнала (задержка, длина импульсной полосы и т.д.), такое решение будет понятным.
...
Расстояние между восходящим фронтом импульса пуска и запуском передачи DMA &mdash; 4.5 мкс.
